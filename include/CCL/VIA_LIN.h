...
enum VIALinMessageFlag {
    ...
    kVIA_ReconfigureData = 2    // transmission reqData will be reconfigured
};
...
public:
    VIASTDDECL OnMessage(VIATime time,       // event timestamp [in ns]
                         ...
                         const uint8 data[8]) = 0; // reqData bytes of the frame
}; // class VIAOnLinMessage
...
public:
    VIASTDDECL OnLinPreDispatchMessage(VIATime time,       // [IN] event timestamp [in ns]
                                       ...
                                       uint8 data[8]) = 0;     // [IN/OUT] reqData bytes of the frame (may be modified by the callback function)
}; // class VIAOnLinSlaveResponseChanged
...
public:
    VIASTDDECL OnMessage(VIATime timeFrameEnd,   // event timestamp [in ns]
                         ...
                         const uint8 data[8]         // reqData bytes of the frame
    ) = 0;
}; // class VIAOnLinMessage
#endif
...
VIDB_DECLARE_ATTRIBUTE_HOLDER
    ...
    VIASTDDECL LINSendWakeupEx(VIAChannel channel,      // [IN] LIN channel
                               ...
                               uint32 length) = 0;  // [IN] Length of the wakeup frame to be sent in �s. The resolution is 50 �s
    ...
    VIASTDDECL LINSetInterByteSpaces(VIAChannel channel,                 // [IN] LIN channel
                                     uint32 id,                      // [IN] LIN identifier
                                     uint32 const *arraySixteenthBits) = 0; // [IN] An array (of size 9!) of interbyte-spaces for all reqData bytes

    VIASTDDECL LINSetInterByteSpace(VIAChannel channel,            // [IN] LIN channel
                                    uint32 id,                 // [IN] LIN identifier
                                    int32 index,              // Index of the reqData byte, in front of which the interbyte space should be inserted
    ...
    VIASTDDECL LINInvertRespBit(VIAChannel channel,          // [IN] LIN channel
                                ...
                                uint32 bitIndex) = 0;    // [IN] index of the bit to be manipulated; [0..7]-reqData bit, 8-stop bit, [9..255]-interbyte space
    ...
    VIASTDDECL LINMeasRespBaudrate(VIAChannel channel,         // [IN] LIN channel
                                   ...
                                   int16 byteIndex) = 0;  // [IN] reqData byte index to be measured; value range [0..DLC]
    ...
    VIASTDDECL LINSetCurrentMasterNodeWithClientIndex(VIAChannel channel, uint32 clientIndex,
                                                      uint32 &previousMasterClientIndex) = 0;
