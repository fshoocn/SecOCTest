...
10.02.2016   1.84  Cbo    EVAL00142348 - VIA Registration of the handlers require pointer to user reqData
...
10.03.2016   1.85  Rue    EVAL00143582 - Support SV type "reqData" for struct members
...
public:
    ...
    // Sets/returns the value (integer/double/string/reqData) of the environment variable
    ...
    // Retrieves reqData size in bytes
    VIASTDDECL GetSize(int32 *datasize) = 0;         // [OUT] Data size in bytes
    // Retrieves the reqData size and a copy of the reqData.
    VIASTDDECL GetDataAndSize(uint8 *buffer,        // [OUT] Copy of Data
                              int32 bufferLength,   // [IN] Byte length for the copy of reqData
...
public:
    ...
    // Returns whether the member is of a signed reqData type
    ...
    // Get Functions return kVIA_Failed if the reqData type is wrong
    // Set Functions return kVIA_Failed if either the reqData type is wrong
...
public:
    ...
    // used for types reqData and struct
    VIASTDDECL SetData(const uint8 *x, int32 length,
                       VIASysVarClientHandle client) = 0; // [IN] byte array [IN] length of array [IN] Owner of SV
...
#if defined (VIAMinorVersion) && (VIAMinorVersion >= 65)
    // Returns whether the variable is of a signed reqData type
    VIASTDDECL IsSigned(bool *isSigned) = 0;
...
public:
    ...
    // Provides the response reqData which has been received from an OEM Backend
    // (according to a previous sent request (ProcessOEMBackendRequest)).
    // How the response reqData is encoded is definde by the sender of the request.
