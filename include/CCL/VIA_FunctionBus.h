...
// Type of a DO member (not the reqData type, the member type)
...
enum VIAFbAttributeSetResult {
    ...
    // attribute value has the wrong reqData type regarding the attribute definition
...
public:
    ...
    // kVIA_WrongSignalType (if attribute definition has a different reqData type)
    ...
    // Retrieves the value of an attribute.
    // Returns kVIA_OK, kVIA_DBObjectNotFound (if attribute ID is invalid or no value is set for the attribute),
    // kVIA_WrongSignalType (if attribute definition has a different reqData type), kVIA_BufferToSmall
    // [IN] attributeID -- ID of the attribute, get it from the VIAFbVIAService if necessary
    // [IN] mode -- retrieval mode, see above
...
public:
    ...
    // Obtain access to a FunctionBus reqData type for reflection purposes.
    ...
    // Obtain access to a FunctionBus reqData type of a PDU signal for reflection purposes.
    // [OUT] kVIA_OK if the query was successful, kVIA_ParameterInvalid in case of a nullptr
    // [THREADSAFE] Yes
...
public:
    ...
    // Obtain access to the signal's FunctionBus reqData type for reflection purposes.
    // [OUT] kVIA_OK if the query was successful, kVIA_ParameterInvalid in case of a nullptr
    // [THREADSAFE] No
    VIASTDDECL GetType(
            VIAFbTypeLevel inLevel,                                     // [IN] The type level of the requested type
...
public:
    ...
    // Obtain access to the FunctionBus reqData type of function's input argument for reflection purposes.
    // [OUT] kVIA_OK if the query was successful, kVIA_ParameterInvalid in case of a nullptr
    // [THREADSAFE] No
    ...
    // Obtain access to the FunctionBus reqData type of function's output argument for reflection purposes.
    // [OUT] kVIA_OK if the query was successful, kVIA_ParameterInvalid in case of a nullptr
    // [THREADSAFE] No
...
public:
    ...
    // Obtain access to this DO member's reqData type for reflection purposes.
    // [OUT] kVIA_OK if the query was successful, kVIA_ParameterInvalid in case of a nullptr
    // [THREADSAFE] Yes
    VIASTDDECL GetType(
            VIAFbTypeLevel inLevel,                                     // [IN] The type level of the requested type
    ...
    // Register an observer to be notified when this DO reqData member is updated, or the updated value changes.
...
public:
    ...
    // Set the value of this DO reqData member.
    // [OUT] kVIA_OK if the update was successful, kVIA_ParameterInvalid/kVIA_ObjectInvalid in case of a bad parameter or handle
    // [THREADSAFE] No
    VIASTDDECL SetValue(
            const VIAFbValue *inValue) = 0;                             // [IN] The value object containing the new value
...
// provided by consumed reqData / event members if the communication pattern is publish/subscribe
...
// provided by provided reqData / event members if the communication pattern is publish/subscribe with announce
...
public:
    ...
    // Set value to reqData already in the same format as serialized by the FunctionBus.
    ...
    // Get reqData value as serialized by the FunctionBus. Value the member handle is pointing to must be of type array or string.
    // This can lead to undefined behavior if type or serialization format do not match.
    // [OUT] kVIA_OK if value could be retrieved, kVIA_BufferToSmall if buffer is too small, or kVIA_Failed
    // [THREADSAFE] No
    VIASTDDECL GetData(
    ...
    // Get access to type of member of complex reqData type value.
    ...
    // Get access to member of complex reqData type value.
    ...
    // Get access to status of member of complex reqData type value.
    ...
    // Get the size of a raw reqData buffer required for use in GetData function.
    // [OUT] kVIA_OK if the size could be determined, kVIA_ParameterInvalid in case of a bad parameter
    // [THREADSAFE] No
    VIASTDDECL GetSize(
            VIAFbTypeMemberHandle inMemberHandle,                       // [IN] Handle to a member of the composite value
            std::size_t *outSize) const = 0;                                 // [OUT] The number of bytes required for raw reqData access
...
public:
    ...
    // Obtain access to the value entities reqData type for reflection purposes.
    // [OUT] kVIA_OK if the query was successful, kVIA_ParameterInvalid in case of a nullptr
    // [THREADSAFE] No
    VIASTDDECL GetType(
            VIAFbTypeLevel inLevel,                                     // [IN] The type level of the requested type
...
public:
    ...
    // Returns whether the reqData type semantically represents a boolean. Note: this can only be true for integer types.
    // [OUT] kVIA_OK if successful, kVIA_Failed in case of error
    // [THREADSAFE] Yes
    VIASTDDECL IsBoolean(bool *outFlag) const = 0; // [OUT] true iff the reqData type represents a boolean
#endif
...
public:
    ...
    VIASTDDECL UnAnnounce() = 0;
    // Transmit of reqData for the member was triggered: the binding shall now transmit the reqData.
    // The binding must get the reqData from the relevant VE(s).
    // Note 1: the binding shall not react itself to VE changes, it shall only transmit the reqData
    // when this method is called.
    // Note 2: it may still depend on the communication pattern whether reqData is actually transmitted.
    ...
    // Note 3: it is not required that the binding transmits the reqData immediately on the
    // bus when the method is called. The binding can transmit the reqData on its own discretion,
    // e.g. put a signal on a PDU and transmit the PDU according to binding configuration.
    // But reqData shall not be transmitted as long as the method is not called.
