...
struct VIASockAddr {
    ...
    union {
        ...
        uint8 rawData[30];  // raw reqData array
    };
};
...
public:
    ...
    // send reqData on a connected socket
    VIASTDDECL Send(void *pParam, int8 buffer[], uint32 size) = 0;

    // send reqData on a unconnected socket
...
public:
    ...
    // is called when reqData is received after receive method was called
