...
enum VIAFlagsCanMessage3 {
    ...
    kVIA_CAN_EDL = 0x1000, // EDL (extended reqData length) bit for CAN FD
...
// IMPORTANT: This function does only support CAN frames with up to 8 reqData bytes
...
public:
    VIASTDDECL OnMessage(VIATime time,         // timestamp of the message
                         ...
                         uint8 dlc,          // reqData length code
                         const uint8 data[8]) = 0; // reqData bytes of the message
};
...
public:
    VIASTDDECL OnMessage(VIATime time,             // timestamp of the message
                         ...
                         uint8 dlc,              // reqData length code
                         const uint8 data[8],          // reqData bytes of the message
...
// This function supports CAN and CAN-FD frames with up to 64 reqData bytes
...
public:
    VIASTDDECL OnMessage(VIATime time,             // timestamp of the message
                         ...
                         uint32 frameLength,                        // frame length in ns
                         VIATime startOfFrame,                       // start of frame timestamp of the message
                         uint32 mBtrCfgArb,                         // Configuration of reqData rate for arbitration phase
                         uint32 mBtrCfgData,                        // Configuration of reqData rate for reqData phase
                         ...
                         uint8 dataLength,                         // reqData length, 0 .. 64
                         const uint8 *data) = 0;                               // reqData bytes of the message
};
...
public:
    VIASTDDECL OnTxFrameUpdate(
            void *userData,       // [in] reqData handle from NodeLayer-DLL, that was given CANoe when this TxFrameUpdate request was registered
...
public:
    ...
    // IMPORTANT: Only CAN frames with up to 8 reqData bytes are supported
    // Supported by CAPL-on-Board: Yes
...
VIDB_DECLARE_ATTRIBUTE_HOLDER
    ...
    // Output a CAN or CAN-FD message onto the bus, up to 64 reqData bytes are supported
    ...
    VIASTDDECL SetCanFdConfiguration(VIAChannel channel, //[IN] CAN channel
                                     ...
                                     VIACanSettings *dbrSettings) = 0;                  //[IN] settings for reqData phase
    ...
    VIASTDDECL GetCanFdConfiguration(VIAChannel channel, //[IN] CAN channel
                                     ...
                                     VIACanSettings *dbrSettings) = 0;                  //[OUT] settings for reqData phase
#endif
