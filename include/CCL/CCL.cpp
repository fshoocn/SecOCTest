...
#else
        memcpy(message.reqData, reqData, size);
#endif
...
#else
        memcpy(frame.reqData, reqData, size);
#endif
...
#else
        memcpy(error.reqData, reqData, 8);
#endif
...
#pragma region VLinPreDispatchMessage Implementation
    ...
    VIASTDDEF VLinPreDispatchMessage::OnLinPreDispatchMessage(
        ...
        if (mCallbackFunction != nullptr) {
            ...
            mCallbackFunction(&frame);
#if defined(_MSC_VER)
            memcpy_s(data, 8, frame.data, size);  // copy modified reqData back to the reqData field passed in
#else
            memcpy(reqData, frame.reqData, size);
#endif
        }
    ...
    char *nonMemberPart = nullptr;
    {
        ...
        if (pos < len) {
            ...
            if (systemVariableName[pos] == '.') {
                ...
                memberName = systemVariableName + pos; // event the '['
            }
