...
public:
    ...
    // so 8 reqData bytes are required to store the value.
    ...
    // For variable of type string or reqData, the parameter maxsize is required
    ...
    // When storing variables of type reqData, first the size of the array is
    //  written as a 32 bit integer value to the memory followed by the reqData
...
public:
    ...
    // Obsolete (version 1.1)
    VIASTDDECL CheckSignalInRange(const char *aSignal, const char *aMsg, const char *aDB, double aLowLimit,
                                  double aHighLimit, int32 *aRet) = 0;
    // Obsolete (version 1.1)
    VIASTDDECL GetSignalGM(const char *aSignal, const char *aMsg, const char *aDB, uint32 aSourceId,
                           double *aValue) = 0;
    // Obsolete (version 1.1)
    VIASTDDECL GetSignalTimeGM(const char *aSignal, const char *aMsg, const char *aDB, uint32 aSourceId,
                               VIATime *aTime) = 0;
    // Obsolete (version 1.1)
    VIASTDDECL CheckSignalInRangeGM(const char *aSignal, const char *aMsg, const char *aDB, uint32 aSourceId,
                                    double aLowLimit, double aHighLimit, int32 *aRet) = 0;
};
